...
`HeyGen (Avatar V)` · `HyperFrames` · `FFmpeg` · `TTS word-timing` · [`Upload-Post`](https://www.upload-post.com/mcp)
...
| **[Upload-Post](https://www.upload-post.com/mcp) account** *(to publish)* | An **`UPLOAD_POST_API_KEY`** (<https://app.upload-post.com> → Settings → API) and a profile with your socials connected (TikTok, Instagram, YouTube, LinkedIn, Threads…). One API for every network — see the [Upload-Post MCP](https://www.upload-post.com/mcp). |
...
one API (and [MCP](https://www.upload-post.com/mcp)) to post to TikTok, Instagram, YouTube, LinkedIn,
Facebook, X, Threads and more. The vertical (with captions) goes to short-form feeds; the horizontal
goes to long-form / professional feeds.
...
- Prefer an MCP/agent flow? The [**Upload-Post MCP**](https://www.upload-post.com/mcp) exposes the same
  publishing as agent tools (note: being remote it can't read local paths — use `open_upload_studio`
  or a public URL; the API script above uploads the local file directly).
